...
// @version      1.3.1
// @description  Сворачиваемое досье искомого кота и окрас поверх поля cw3.
// @author       local
...
/* Overlay for Tampermonkey. Do not add GM_* APIs. */
...
(function () {
  ...
  function hookWebSocket() {
    ...
    const hookUrl = getExtensionFileUrl("ws-hook.js");
    if (hookUrl) {
      script.src = hookUrl;
    ...
    if (!hookUrl) script.remove();
  }

  function getExtensionFileUrl(filename) {
    try {
      if (
        ...
        if (isExtensionUrl(url)) return url;
      }
    } catch (e) {}
    try {
      if (
  ...
  function isExtensionUrl(url) {
    return (
      typeof url === "string" &&
      /^(chrome|moz|safari|ms-browser)-extension:/.test(url)
    );
  }
